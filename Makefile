...
.PHONY: help vagrant clean configure proton install deploy module protonsdk push-protonsdk
...
push-protonsdk: vagrant
	vagrant ssh -c 'docker login registry.gitlab.steamos.cloud && make -C proton/docker $(UNSTRIPPED) $(CCACHE_FLAG) PROTONSDK_VERSION=$(protonsdk_version) proton push'
