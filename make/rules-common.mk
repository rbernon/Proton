...
$(2)_DEPS := $$(call toupper,$$($(2)_DEPENDS))
...
$$(OBJ)/.$(1)-configure$(3): $$(patsubst %,%-build$(3),$$($(2)_DEPENDS))
...
$(2)_ENV$(3) = \
    ...
    PATH="$$(call list-join,:,$$(foreach d,$$($(2)_DEPS),$$($$(d)_BINDIR$(3))),,:):$$(SRC)/glslang/bin:$$$$PATH" \
    LD_LIBRARY_PATH="$$(call list-join,:,$$(foreach d,$$($(2)_DEPS),$$($$(d)_LIBDIR$(3))),,:)$$$$LD_LIBRARY_PATH" \
    PKG_CONFIG_PATH="$$(call list-join,:,$$(foreach d,$$($(2)_DEPS),$$($$(d)_LIBDIR$(3))/pkgconfig))" \
    CFLAGS="$$(foreach d,$$($(2)_DEPS),-I$$($$(d)_INCDIR$(3))) $$($(2)_CFLAGS) $$(COMMON_FLAGS) $$(COMMON_FLAGS$(3))" \
    CXXFLAGS="$$(foreach d,$$($(2)_DEPS),-I$$($$(d)_INCDIR$(3))) $$($(2)_CXXFLAGS) $$(COMMON_FLAGS) $$(COMMON_FLAGS$(3)) -std=c++17" \
    LDFLAGS="$$(foreach d,$$($(2)_DEPS),-L$$($$(d)_LIBDIR$(3))) \
             $$(foreach d,$$($(2)_DEPS),-Wl,-rpath-link=$$($$(d)_LIBDIR$(3))) \
             $$($(2)_LDFLAGS) $$(LDFLAGS)"

endef

ifneq ($(UNSTRIPPED_BUILD),)
install-strip = objcopy --only-keep-debug $(1) $(2)/$(notdir $(1)).debug && \
                objcopy --add-gnu-debuglink=$(2)/$(notdir $(1)).debug --strip-debug $(1) $(2)/$(notdir $(1))
else
install-strip = objcopy --strip-debug $(1) $(2)/$(notdir $(1)) && rm -f $(2)/$(notdir $(1)).debug
endif
